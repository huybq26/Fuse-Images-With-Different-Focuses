...
class SESF_Fuse():
    ...
    def __init__(self, attention='cse'):
        ...
        self.model_path = os.path.join(
            os.getcwd(), "nets", "parameters", "lp+lssim_se_sf_net_times30.pkl")
    ...
    def fuse(self, img1, img2):
        ...
        img1_tensor = self.data_transforms(
            img1_gray_pil).unsqueeze(0).to(self.device)
        img2_tensor = self.data_transforms(
            img2_gray_pil).unsqueeze(0).to(self.device)

        dm = self.model.forward("fuse", img1_tensor,
                                img2_tensor, kernel_radius=self.kernel_radius)
        # Morphology filter and Small region removal
        h, w = img1.shape[:2]
        # 'disk' kernel with ks size for structural element
...
class SESFuseNet(nn.Module):
    ...
    def forward(self, phase, img1, img2=None, kernel_radius=5):
        ...
        if phase == 'train':
            ...
            output = self.fusion_channel_sf(
                se_cat3_1, se_cat3_2, kernel_radius=kernel_radius)
        return output
    ...
    def fusion_channel_sf(f1, f2, kernel_radius=5):
        ...
        f1_grad = torch.pow((f1_r_shift - f1), 2) + \
            torch.pow((f1_b_shift - f1), 2)
        f2_grad = torch.pow((f2_r_shift - f2), 2) + \
            torch.pow((f2_b_shift - f2), 2)

        kernel_size = kernel_radius * 2 + 1
        add_kernel = torch.ones(
            (c, 1, kernel_size, kernel_size)).float()
        ...
        # get decision map
        dm_tensor = torch.where(
            f1_sf > f2_sf, weight_ones, weight_zeros)
